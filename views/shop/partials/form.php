		...
		<div class="row">
			...
			<div class="col-md-4 collection">
				...
				<div class="times" style="margin-bottom: 10px;">
					<select name="times[0][day]" id="days" class="form-control">
				  <?php foreach (\App\Entity\Time::DAYS as $key => $day): ?>
						<option value="<?= $key ?>"><?= $day ?></option>
				  <?php endforeach; ?>
					</select>
					<input type="text" class="form-control" name="times[0][start_hours]" placeholder="Crénaux d'ouverture">
					<input type="text" class="form-control" name="times[0][end_hours]" placeholder="Crénaux de fermeture">
				</div>
			</div>
			<a href="" class="btn btn-success" id="add_collection">+</a>
		</div>
    </div>
</form>
